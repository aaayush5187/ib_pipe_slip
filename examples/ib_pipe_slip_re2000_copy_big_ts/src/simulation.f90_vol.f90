  ...
  subroutine postproc_vel()
   ...
   subroutine simulation_init
      ...
      create_sgs: block
        ...
        call fs%cfg%sync(VFy)
        call fs%cfg%sync(VFz)
      ...
      do while (.not.time%done())
         ...
         sgs_modeling: block
           ...
           do k=fs%cfg%kmin_,fs%cfg%kmax_
              do j=fs%cfg%jmin_,fs%cfg%jmax_
                 do i=fs%cfg%imin_,fs%cfg%imax_
                    Uib(i,j,k)=sum(fs%itpr_x(:,i,j,k)*sgs%uslip(i-1:i,j,k))
                    Vib(i,j,k)=sum(fs%itpr_y(:,i,j,k)*sgs%vslip(i,j-1:j,k))
                    Wib(i,j,k)=sum(fs%itpr_z(:,i,j,k)*sgs%wslip(i,j,k-1:k))
                 end do
              end do
           end do
         ...
         do while (time%it.le.time%itmax)
            ...
            !     do j=fs%cfg%jmin_,fs%cfg%jmax_
            !        do i=fs%cfg%imin_,fs%cfg%imax_
            !           resU(i,j,k)=resU(i,j,k)+(1.0_WP-VFx(i,j,k))*(Uib(i,j,k)-fs%U(i,j,k))
            !           resV(i,j,k)=resV(i,j,k)+(1.0_WP-VFy(i,j,k))*(Vib(i,j,k)-fs%V(i,j,k))
            !           resW(i,j,k)=resW(i,j,k)+(1.0_WP-VFz(i,j,k))*(Wib(i,j,k)-fs%W(i,j,k))
            !        end do
            !     end do
            !  end do
            !end block ibforcing2
            ...
            ibforcing: block
              ...
              do k=fs%cfg%kmin_,fs%cfg%kmax_
                 do j=fs%cfg%jmin_,fs%cfg%jmax_
                    do i=fs%cfg%imin_,fs%cfg%imax_
                       fs%U(i,j,k)=VFx(i,j,k)*fs%U(i,j,k)+(1.0_WP-VFx(i,j,k))*Uib(i,j,k)
                       fs%V(i,j,k)=VFy(i,j,k)*fs%V(i,j,k)+(1.0_WP-VFy(i,j,k))*Vib(i,j,k)
                       fs%W(i,j,k)=VFz(i,j,k)*fs%W(i,j,k)+(1.0_WP-VFz(i,j,k))*Wib(i,j,k)
                    end do
                 end do
              end do
              call fs%cfg%sync(fs%U)
            ...
            fs%P=fs%cfg%VF*fs%P+fs%psolv%sol
